...
logging.basicConfig(
        level=logging.DEBUG,
        filename=PROJ_DIR/f'log/make_operation/{TODAY}.log',
        format='%(asctime)s\n%(message)s'
    )
...
入力: (focus_att_l,focus_att2func_d,drilldown_att_l)
    - focus_att_l: 注目属性(計算を行う属性)のリスト, ["売上","従業員数"]
    - focus_att2func_d: define_aggregation_Fの出力, {"売上":[1,3,4],"従業員数":[1,3,4],"上場flg":[9]}
出力: {(注目属性, Aggregation_F):演算子, ...}
"""
def define_scalar_arithmetic_operator(focus_att_l, focus_att2func_d):
    return_d = {}
    for focus_att in focus_att_l:
        for func_num in focus_att2func_d[focus_att]:
            if(func_num<10):
                return_d[(focus_att,func_num)]=[3,4] # 数字の意味についてはOPERATOR_Dを参照
            else:
